...
**Nothing is signed.** The official catalog has zero signatures across all published versions and an empty `trustedSigners` list. Every guarantee above is about *routing* — that a result reaches the right caller, that a provider cannot forge or enumerate requests. None of it is about *identity*. A module name is a string a package chose for itself: two packages can claim the same `provides` and the same display name, and the chooser cannot tell you which is the wallet you installed last week. What the chooser does about that is partial and worth knowing exactly. Every row shows the package name under the display name, and so does the line naming the requester — a package called `evil_ui` shipping `"display_name": "Wallet"` cannot hide behind the label. "Details" expands in place (it does not navigate away, and does not end the request) to show version, originating repository, install type, and an explicit **"Unsigned — the shell cannot confirm who published this"**. The chooser also lists catalog packages that could service the intent and are not installed, under a **"Not installed"** heading with an `Install…` affordance rather than a clickable row — they do the opposite of the rows above, ending the request (`cancelled`) before the install begins, because an uninstalled package cannot be dispatched to and holding the request open across a download would put the transfer's duration into its lifetime. Before this the mixed case rendered identically to "this is the only app that can do this", and nothing else in the UI surfaces what a package provides. Install offers likewise name the repository a package would come from. Neither dialog derives that itself: `MainUIBackend` resolves the catalog URL against the repository registry and hands down a `repositoryLabel` and a `repositoryLink` alongside it (`app/RepositorySource.h`). The label is the same name the App Manager and Settings show, owner-qualified when two repositories claim it. The link is the repository's declared `homepage` where it has one, otherwise `github.com/{sourceOwner}/{sourceRepo}` — fields logos-package-downloader already parsed out of the URL — because the *hostname* is identical for every package published through GitHub and so identifies nobody. A host the downloader does not recognise keeps its own URL rather than being resolved to a guess. Both dialogs render the link selectable with a copy button: checking a source happens in another window, and a link that must be retyped by eye is one nobody checks. All of that is provenance for the *channel*, never the *publisher*, and none of it is proof.
