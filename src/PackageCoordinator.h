...
private:
    ...
    // A module that is not loaded at construction may be loaded later in the
    // session, so the subscriptions are retried on coreModulesChanged(); these
    // keep the "not loaded" warning to one line rather than one per
    // stats-timer tick.
