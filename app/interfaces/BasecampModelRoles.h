...
// them live in the shell. The proxies need the role NUMBERS and nothing else —
// no constructor, no method, no symbol of any kind. So the numbers live here,
// in the same interfaces/ directory as IShellHost, and both sides include this
// one copy.
//
// The models inherit these structs rather than redeclaring the enum, so
// `AppsModel::NameRole` keeps resolving exactly as before for every host-side
// call site; the shell spells the same constant `AppsModelRoles::NameRole`.
// There is one definition either way.
//
...
// reading the wrong column rather than anything that fails to build.
// ─────────────────────────────────────────────────────────────────────────────
